  ...
  <div class="modal z-50">
    ...
    >
      <nav class="flex justify-between">
        ...
        <ul class="flex">
          <li @click="checkTaskHandler" class="mr-2 cursor-pointer">標記為已完成</li>
          <li @click="deleteTaskHandler" class="mr-2 cursor-pointer">刪除</li>
          <li>
            <img
              @click="closeModal"
...
const checkTaskHandler = async () => {
  try {
    await todoStore.checkedItem({ taskId: editedTask.value._id });
    resetTask();
  } catch (error) {
...
const deleteTaskHandler = async () => {
  try {
    await todoStore.deleteItem({ taskId: editedTask.value._id });
    emit("close");
  } catch (error) {

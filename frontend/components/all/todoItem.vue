<template>
  <div class="all-todo-item h-[100vh] m-2">
    <div class="rounded-tl-lg rounded-tr-lg max-h-[71vh] overflow-y-auto bg-white p-4">
      <ul class="mb-4">
        <h2 class="text-blue-500 font-bold mb-2">今天</h2>
        <li
      ...
      <ul class="mb-4">
        <h2 class="text-blue-500 font-bold mb-2">明日</h2>
        <li
          v-for="item in tomorrowTodoItems"
          :key="item._id"
          class="w-list shadow-lg mx-auto p-2 mb-2 rounded-lg hover:bg-gray-200"
          :class="{ 'bg-gray-200': nowSelected === item._id }"
        >
          <div class="flex items-center">
            <div
            ...
            <div
              @click="editTaskHandler(item._id, 'tomorrow')"
              class="text-left ml-2 cursor-pointer"
            >
              <div :class="{ 'line-through': item.checked }">{{ item.name }}</div>
              <div class="flex items-center text-sm text-gray-500">
      ...
      <ul class="mb-4">
        <h2 class="text-blue-500 font-bold mb-2">即將到來</h2>
        <li
          v-for="item in futureTodoItems"
          :key="item._id"
          class="w-list shadow-lg mx-auto p-2 mb-2 rounded-lg hover:bg-gray-200"
          :class="{ 'bg-gray-200': nowSelected === item._id }"
        >
          <div class="flex items-center">
            ...
            <div
              @click="editTaskHandler(item._id, 'future')"
              class="text-left ml-2 cursor-pointer"
            >
              <div :class="{ 'line-through': item.checked }">{{ item.name }}</div>
              <div class="flex items-center text-sm text-gray-500">
      ...
      <div class="flex justify-around mb-4">
        <div class="flex">
          <img
            ...
            src="../../public/blue-list.svg"
            alt=""
          />
          <h5 class="text-blue-500" v-if="nowCategory">{{ nowCategory }}</h5>
        </div>
        |
        <div class="flex">
          <img
            ...
            alt=""
          />
          <h5 class="text-blue-500" v-if="nowTag">{{ nowTag.name }}</h5>
        </div>
      </div>
      <div
    ...
    <allCategory
      v-if="showCategory"
      @editCategory="setCategory"
      @close="showCategory = false"
    />
    <allTag v-if="showTag" @editTag="setTag" @close="showTag = false" />
  </div>
</template>
...
const todayTodoItems = computed(() => todoStore.todoItems?.today || []);
const tomorrowTodoItems = computed(() => todoStore.todoItems?.tomorrow || []);
const futureTodoItems = computed(() => todoStore.todoItems?.future || []);
...
const submitTaskHandler = async () => {
  try {
    await todoStore.addTodoItem({
      ...
      category: nowCategory.value || 'Personal',
      tags: nowTag.value,
    });
    nowCategory.value = ''
    nowTag.value = ''
